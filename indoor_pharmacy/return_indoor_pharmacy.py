...
class return_indoor_pos_order(osv.osv):
    _name = "return.indoor.pos.order"
    _description = "Point of Sale for Return Indoor"
    _order = "id desc"
    ...
    _columns = {
        ...
        'lines': fields.one2many('return.indoor.pos.order.line', 'order_id', 'Order Lines',required=True),
        ...
        'return_prescription_item': fields.many2one('hospital.admission', "Medicine Info"),
        }
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        res =super(return_indoor_pos_order, self).create(cr, uid, vals, context)

        serial_no = 'RPS-0'+str(res)

        #name_text = 'A-0' + str(stored)
        cr.execute('update return_indoor_pos_order set name=%s where id=%s', (serial_no, res))
        cr.commit()
        return res
    ...
    def order_confirm(self, cr, uid, ids, context=None):
        ...
        self_obj = self.pool.get('return.indoor.pos.order').browse(cr, uid, ids, context=None)
        ...
        general_admission_ids = self.pool.get("hospital.admission").search(cr, uid, [
            ('id', '=', self_obj.general_admission_id.id)], context=None)
        general_admission_obj = self.pool.get('hospital.admission').browse(cr, uid, general_admission_ids, context=None)
        # general_admission_obj.return_prescription_id = self_obj

        p_line_ids = [itm.id for itm in general_admission_obj.return_prescription_id]
        p_line_ids.append(self_obj.id)
        general_admission_obj.return_prescription_id = p_line_ids

        self_obj.state = 'confirm'


class return_indoor_pos_order_line(osv.osv):
    _name = "return.indoor.pos.order.line"
    ...
    _columns = {
        ...
        'order_id': fields.many2one('return.indoor.pos.order', 'Order Ref', ondelete='cascade'),
